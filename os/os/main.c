...
#define MAX_CPUBURST 1000
#define MAX_ARRIVETIME 500
#define IO 10
#define MAX_IO 100
   ...
   for(i=0; i<PROCESS_NUM; i++)
    ...
    else // parent process
    {
      ...
      { 
        ...
        {
          if(!(ptr->preemptive))
            j = ptr -> cpu_burst; // ptr is preemptive
          else if(ptr->remain_time < QUANTUM)
            j = ptr->remain_time;
          else
            j = QUANTUM;
          ...
          ptr->execute_time += j;
          ptr->remain_time -= j;
          fprintf(fp1, "%d %d %d %d %d %d %d %d %d %d %d %d\n", context_switch, ptr->pid, ptr->state, ptr->cpu_burst, ptr->arrive_time, ptr->waiting_time, ptr->execute_time, ptr->remain_time, ptr->finish_time, ptr->io, ptr->io_time, ptr->preemptive);
          if(!(ptr->remain_time))
          {
            ptr->finish_time = t;
            ptr->waiting_time = t - (ptr->execute_time);
