...
int main(void)
{
  int i, j, fds[PROCESS_NUM][2], cpuburst, fds1[PROCESS_NUM][2], fds2[PROCESS_NUM][2], tempor;
  char * tempbuf[8];
  //int fds[2];
  const int PAGE_RESET = (MEMORY_SIZE / PAGE_SIZE);
  static int t, context_switch, sum_burst, sum_io, p, mv;
  PCB pcb[PROCESS_NUM];
  PCB * ptr = NULL, ** ptr1 = NULL, * ptr2 = NULL, * temp = NULL, * cptr = NULL;
  void * nptr = NULL;
  ...
  */
  ...
  for(i=0; i<PROCESS_NUM; i++)
    pipe(fds[i]), pipe(fds1[i]), pipe(fds2[i]);
  //pipe(fds);

  FILE * fp = fopen("os.dump", "wt");
  FILE * fp1 = fopen("log.dump", "wt");
  FILE * fp3 = fopen("page.dump", "wt");
  
  for(i=0; i<MEMORY_SIZE/PAGE_SIZE; i++)
    pt[i].use = 0, pt[i].copy_on_write = 0;
  ...
  if(!pid) // child process
  {
    ...
    while(1)
    {
      if(ptr2->remain_time == 0)
        break;
      write(fds1[temp][1], ptr1, 8);
      read(fds2[temp][0], (void*)buf, 8);
    }
  ...
  else // parent process
  {
    ...
    { 
      ...
      while(1)
      {
        ...
        ptr->execute_time += j;
        ptr->remain_time -= j;
      	read(fds1[pn[ptr->pid]][0], (void*)tempbuf, 8);
        write(fds2[pn[ptr->pid]][1], &cptr, 8);

        for(i=0; i<REQ_PAGE; i++)
        {
          if(pt[p].use)
          {
            if(pt[p].copy_on_write)
              pt[p].use = 0; 
          }
          pt[p].physicalm = p * PAGE_SIZE;
          m[ pt[p].physicalm ] = mv++;
          pt[p].use = 1;
          pt[p].copy_on_write = rand() % 2;
          if(p >= PAGE_RESET)
            p = 0;
          else
            p++;
        }
        fprintf(fp3, "%d %d %d\n", ptr->pid, p, p+REQ_PAGE);
